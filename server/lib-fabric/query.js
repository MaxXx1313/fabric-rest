 ...
 * @param {string} org
 */
const getBlockList = function(peer, channelID, lastIndex, params, username, org) {
  var targetPeer  = getPeer(peer, org);
  ...
  //
  const _hashList = [];
  function _getHashRecursive(channel, index, limit) {
    if(limit <= 0) {
      return Promise.resolve(_hashList);
    }
    ...
    logger.debug('_getHashRecursive: ' + index);
    // return channel.queryBlockByHash(Buffer.from(hash, 'base64'), targetPeer)
    return channel.queryBlock(index, targetPeer)
      .then( blockInfo => {
        _hashList.push({
          dataHash: blockInfo.header.data_hash,
          index: blockInfo.header.number
        });
        return _getHashRecursive(channel, index-1, limit-1);
      });
  }
    ...
    .then(_channel => {
      ...
      if (lastIndex) {
        ...
        return channel.queryInfo(targetPeer)
          .then((blockchainInfo) => {
          	// logger.debug('blockchainInfo', blockchainInfo);
          	// TODO: 'blockchainInfo.height' is a Long, but so far we ignore 'high' part of it
          	return parseInt(blockchainInfo.height.low);
          	// return channel.queryBlockByHash(blockchainInfo.currentBlockHash.toString('base64'), targetPeer)
          })
          // .then((lastBlock) => parseInt(lastBlock.header.number) + 1);
      }
    }).then((lastIndex) => {
      return _getHashRecursive(channel, lastIndex - 1, limit);
    }).catch((err) => {
      logger.error('Failed to query with error:' + err.stack ? err.stack : err);
      throw new Error('Failed to query with error:' + err.stack ? err.stack : err);
    });
